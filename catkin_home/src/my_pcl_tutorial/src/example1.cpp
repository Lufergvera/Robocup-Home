...
void segmentation::cloud_cb(const sensor_msgs::PointCloud2ConstPtr &input)
{
  ...
  pcl_conversions::toPCL(*input, *cloud_filtered2);
  ...
  ROS_INFO_STREAM(temp->points.size());
  ...
  //Line for test
  float dumb[2] = {0.0f, 0.0f};

  THEcluster = BiggestCluster(PlannedCloud, tolerance, flag);
